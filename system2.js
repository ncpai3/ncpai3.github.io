...
const System2 = (() => {
    ...
    // --- CORE LOGIC: PATTERN MATCHER (HT 2) ---
    class PatternMatcher {
        constructor() {
            this.MIN_HISTORY_FOR_ANALYSIS = 10;
            this.MIN_PATTERN_LENGTH = 10;
            this.MAX_PATTERN_LENGTH = 15; // <<<< HT 2
    ...
    function getConfidenceSequenceData(allSessions) {
        ...
        for (const session of allSessions) {
            ...
            const sessionPredictions = session.sys2.predictions || []; // Sửa: sys2
    ...
    // --- CHỐT ANALYSIS LOGIC (HT 2) ---
    // (Logic Chốt của HT 2 giống hệt HT 3, khác HT 1)
    ...
    function getChotAnalysisData(allSessions) {
        ...
        for (const session of allSessions) {
            ...
            (session.sys2.verdictPredictions || []).forEach(p => { // Sửa: sys2
                const result = p.predicted === p.actual ? 'W' : 'L';
            ...
            (session.sys2.predictions || []).forEach(p => { // Sửa: sys2
                const result = (p.predicted === p.actual) ? 'W' : 'L';
                const foundRange = CONFIDENCE_RANGES.find(r => p.confidence >= r.min && p.confidence < r.max);
                if (foundRange) {
    ...
    // --- UI UPDATE FUNCTIONS (Cho HT 2) ---

    function updateAccuracyUI(session) {
        const predictions = session.sys2.predictions || []; // Sửa: sys2
        const stats = getPredictionStats(predictions);
        // SỬA ID: Thêm "-2"
        document.getElementById('accuracyRate-2').textContent = `${stats.rate.toFixed(1)}%`;
        document.getElementById('correctPredictions-2').textContent = stats.correct;
        document.getElementById('totalPredictions-2').textContent = stats.total;
    }

    function updateGlobalAccuracyUI(allSessions) {
        const allPredictions = allSessions.flatMap(s => s.sys2.predictions || []); // Sửa: sys2
        const stats = getPredictionStats(allPredictions);
        // SỬA ID: Thêm "-2"
        document.getElementById('globalAccuracyRate-2').textContent = `${stats.rate.toFixed(1)}%`;
        document.getElementById('globalCorrectPredictions-2').textContent = stats.correct;
        document.getElementById('globalTotalPredictions-2').textContent = stats.total;
    }
    ...
    function updateAnalysisUI(session, allSessions) {
        const { prediction, recommendation, analysisText, confidence, isCertain } = matcher.getPrediction(
            ...
            allSessions,
            {
                optimalLength: session.sys2.optimalLength, // Sửa: sys2
                lengthPerformance: session.sys2.lengthPerformance // Sửa: sys2
            }
        );
        ...
        }"><strong>Phán Quyết HT 2:</strong> ${isCertain ?
        ...
        // SỬA ID: Thêm "-2"
        const analysisResultDiv = document.getElementById('analysisResult-2');
        const analysisCard = document.getElementById('analysis-card-2');
        
        analysisCard.classList.remove('card-glow-p', 'card-glow-b', 'card-glow-certain');
        
        let optimalLengthDisplay = session.sys2.optimalLength ? `<div class="mb-4 p-3 bg-gray-900 rounded-lg text-center text-cyan-300 text-sm"><span class="font-semibold">Chiều dài tối ưu:</span><span class="text-lg font-bold ml-2">${session.sys2.optimalLength}</span></div>` : '';
        ...
        if (prediction) {
            ...
            <div class="text-center bg-gray-900 py-6 rounded-lg border-2 ${finalPredictionForDisplay === 'P' ? 'border-blue-500' : 'border-red-500'}">
                <p class="text-lg text-gray-400 mb-2">Đề xuất theo hệ thống 2:</p>
            ...
            <div class="bg-gray-700 p-4 rounded-lg mt-4">
                <p class="font-semibold text-gray-300">Phân tích ban đầu (HT 2):</p>
    ...
    function updateChotAnalysisUI(session, allSessions) {
        ...
        // SỬA ID: Thêm "-2"
        const finalChotResultDiv = document.getElementById('finalChotResult-2');
        const finalChotResultText = document.getElementById('finalChotResultText-2');
        const chotAnalysisDetailsDiv = document.getElementById('chotAnalysisDetails-2');
        const chotHistoryChartDiv = document.getElementById('chotHistoryChart-2');
        const chotAccuracyStatsSpan = document.getElementById('chotAccuracyStats-2');
        ...
        if (originalSystemPB && finalVerdictInfo !== null) {
            ...
            for (const source of sources) {
                ...
                if (analysisResult.predictedWL && isActive) {
                    ...
                    // Logic bỏ phiếu của HT2/HT3 (khác HT1)
            ...
            chotAnalysisDetailsDiv.innerHTML = '<p class="text-gray-500 text-center py-4">Chờ tín hiệu từ Phán Quyết HT 2...</p>';
        }
        ...
        const chotHistory = session.sys2.chotPredictions || []; // Sửa: sys2
        let correctChot = 0;
        ...
        if (historyToShow.length > 0) {
            ...
            chotHistoryChartDiv.innerHTML = '<p class="text-gray-600 text-xs text-center w-full">Chưa có dữ liệu lịch sử Chốt 2...</p>';
        }
    ...
    function updateLengthPerformanceAndFindOptimal(history, allSessions, newResult, session) {
        if (!session.sys2.lengthPerformance) session.sys2.lengthPerformance = {}; // Sửa: sys2
        const candidates = [];
        
        // Loop từ 10 (theo logic HT 2)
        for (let len = 10; len <= 20; len++) {
            ...
            if (prediction) {
                if (!session.sys2.lengthPerformance[len]) session.sys2.lengthPerformance[len] = { wins: 0, total: 0 };
                const stats = session.sys2.lengthPerformance[len]; // Sửa: sys2
                stats.total++;
        ...
        if (candidates.length === 0) return session.sys2.optimalLength; // Sửa: sys2
    ...
    return {
        ...
        getInitialState: () => ({
            ...
            lengthPerformance: {},
            optimalLength: null
            // HT2 không có chotLengthPerformance
        }),
        ...
        commitResult: (session, result, allSessions) => {
            ...
            if (tempState.currentFinalPrediction && tempState.currentFinalPrediction.prediction) {
                session.sys2.verdictPredictions.push({
            ...
            if (chotPredictionForThisRound) { // Chỉ lưu P/B
                session.sys2.chotPredictions.push({
            ...
            session.sys2.optimalLength = newOptimalLength;
        ...
        undoLast: (session, allSessions, lastResult) => {
            ...
            // --- Hoàn tác HT2 Performance ---
            if (session.sys2.lengthPerformance) {
                for (let len = 10; len <= 20; len++) {
                    ...
                    if (prediction) {
                        const stats = session.sys2.lengthPerformance[len];
            ...
            // Tính lại optimalLength HT2
            const candidatesHt = [];
            if (session.sys2.lengthPerformance) {
                for (const len in session.sys2.lengthPerformance) {
                    const stats = session.sys2.lengthPerformance[len];
                    if (stats.total > 0) {
            ...
            if (candidatesHt.length > 0) {
                ...
                session.sys2.optimalLength = candidatesHt[0].length;
            } else {
                session.sys2.optimalLength = null;
            }

            // Pop predictions
            if (session.sys2.predictions.length > 0) session.sys2.predictions.pop();
            if (session.sys2.verdictPredictions.length > 0) session.sys2.verdictPredictions.pop();
            if (session.sys2.chotPredictions.length > 0) session.sys2.chotPredictions.pop();
        },
        ...
        clearHistory: (session) => {
            session.sys2 = System2.getInitialState();
        }
    };
